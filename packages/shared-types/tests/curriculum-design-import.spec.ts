...
import {
  ...
  CurriculumImportReviewArtifactSchema,
  Grade,
  Subject,
...
describe("curriculum design import schema", () => {
  it("accepts a structured AI-native curriculum import draft", () => {
    expect(CurriculumDesignImportDraftSchema.safeParse(buildValidDraft()).success).toBe(true);
  });

  it("rejects markdown-only or incomplete drafts without runtime contract", () => {
    const invalid = structuredClone(buildValidDraft());
    delete (invalid as Partial<CurriculumDesignImportDraft>).runtime_contract;

    expect(CurriculumDesignImportDraftSchema.safeParse(invalid).success).toBe(false);
  });

  it("rejects student blocks without target knowledge nodes", () => {
    const invalid = structuredClone(buildValidDraft());
    invalid.student_material.pages[0]!.blocks[0]!.target_knowledge_nodes = [];

    expect(CurriculumDesignImportDraftSchema.safeParse(invalid).success).toBe(false);
  });

  it("rejects review artifacts that try to apply blocked imports", () => {
    const result = CurriculumImportReviewArtifactSchema.safeParse({
      artifact_id: "artifact_import_review_001",
      schema_version: "edu-ai-curriculum-import-review-v0.1",
      source_import_id: "import_math_g8_linear_function_001",
      target_unit_id: "math_g8_linear_function_intro",
      status: "blocked",
      candidate_patches: [],
      future_extensions: [],
      validation_summary: {
