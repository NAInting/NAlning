...
describe("provider execution follow-up receipt", () => {
  ...
  const executionFailedReceipt: UnitReviewProviderExecutionReceipt = {
  ...
  const executionFailedReconciliation: UnitReviewProviderExecutionReconciliation = {
  ...
  it("builds a no-op receipt when execution success already handed off to the artifact contract", () => {
    const successFollowUp: UnitReviewProviderExecutionFollowUp = {
    ...
    const plan = buildUnitReviewProviderExecutionFollowUpPlan(successFollowUp);
    const receipt = buildUnitReviewProviderExecutionFollowUpReceipt(plan, {
      executed_at: "2026-04-24T00:30:00.000Z",
    });
  ...
  it("builds an executor-ready upsert receipt for manual execution triage", () => {
    const plan = buildFailedPlan();
    const receipt = buildUnitReviewProviderExecutionFollowUpReceipt(plan, {
      executed_at: "2026-04-24T00:30:00.000Z",
      upsert_status: "already_applied",
    });
  ...
  it("rejects receipt rendering when the source follow-up delivery plan is tampered", () => {
    const tamperedPlan = {
      ...buildFailedPlan(),
      final_follow_up_item_key: "wrong-item-key",
    };

    const validation = validateUnitReviewProviderExecutionFollowUpReceiptSource(
      baseRequest,
      approvedDecision,
      authorizedAttempt,
      executionFailedReceipt,
      executionFailedReconciliation,
      executionFailedFollowUp,
      tamperedPlan
    );
    ...
    expect(validation.issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: "source_follow_up_plan_payload_mismatch" }),
      ])
    );
  });

  it("detects tampered receipt payloads during validation", () => {
    const plan = buildFailedPlan();
    const receipt = buildUnitReviewProviderExecutionFollowUpReceipt(plan, {
      executed_at: "2026-04-24T00:30:00.000Z",
    });
    ...
    const validation = validateUnitReviewProviderExecutionFollowUpReceipt(plan, receipt);
