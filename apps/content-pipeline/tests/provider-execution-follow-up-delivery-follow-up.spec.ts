...
describe("provider execution follow-up delivery follow-up", () => {
  ...
  function buildManualTriagePlan(): UnitReviewProviderExecutionFollowUpPlan {
    return buildUnitReviewProviderExecutionFollowUpPlan(failedFollowUp);
  }

  function buildManualTriageReceipt(
    upsertStatus?: "applied" | "already_applied" | "failed"
  ): UnitReviewProviderExecutionFollowUpReceipt {
    return buildUnitReviewProviderExecutionFollowUpReceipt(buildManualTriagePlan(), {
      executed_at: "2026-04-24T00:35:00.000Z",
      ...(upsertStatus ? { upsert_status: upsertStatus } : {}),
    });
  }

  function buildArtifactPlan(): UnitReviewProviderExecutionFollowUpPlan {
    return buildUnitReviewProviderExecutionFollowUpPlan(artifactFollowUp);
  }

  function buildArtifactReceipt(): UnitReviewProviderExecutionFollowUpReceipt {
    return buildUnitReviewProviderExecutionFollowUpReceipt(buildArtifactPlan(), {
      executed_at: "2026-04-24T00:35:00.000Z",
    });
  }

  it("keeps the chain closed when the source branch is a trusted result-artifact handoff", () => {
    ...
    const followUpDeliveryFollowUp =
      buildUnitReviewProviderExecutionFollowUpDeliveryFollowUp(plan, reconciliation);

    expect(followUpDeliveryFollowUp).toMatchObject({
      schema_version:
  ...
  it("closes the chain when a manual triage follow-up item was delivered successfully", () => {
    const plan = buildManualTriagePlan();
    const receipt = buildManualTriageReceipt();
    const reconciliation = buildUnitReviewProviderExecutionFollowUpReconciliation(plan, receipt);
    const followUpDeliveryFollowUp =
      buildUnitReviewProviderExecutionFollowUpDeliveryFollowUp(plan, reconciliation);
    ...
    expect(followUpDeliveryFollowUp).toMatchObject({
      ...
      follow_up_delivery_chain_closed: true,
      active_follow_up_item: {
        item_key: expect.stringContaining("provider-execution"),
        human_queue: "manual_triage_queue",
  ...
  it("routes to manual repair when follow-up delivery failed", () => {
    const plan = buildManualTriagePlan();
    const receipt = buildManualTriageReceipt("failed");
    const reconciliation = buildUnitReviewProviderExecutionFollowUpReconciliation(plan, receipt);
    const followUpDeliveryFollowUp =
      buildUnitReviewProviderExecutionFollowUpDeliveryFollowUp(plan, reconciliation);
    ...
    expect(followUpDeliveryFollowUp).toMatchObject({
      ...
      follow_up_action: "open_manual_repair_follow_up_delivery_item",
      follow_up_delivery_chain_closed: false,
      active_follow_up_item: {
        item_key: null,
        human_queue: null,
        should_remain_open: false,
  ...
  it("routes to receipt triage when the follow-up receipt is not trustworthy", () => {
    const plan = buildManualTriagePlan();
    const receipt = buildManualTriageReceipt();
    receipt.final_follow_up_item_key = "forged-follow-up-item";
    const reconciliation = buildUnitReviewProviderExecutionFollowUpReconciliation(plan, receipt);
    const followUpDeliveryFollowUp =
      buildUnitReviewProviderExecutionFollowUpDeliveryFollowUp(plan, reconciliation);
  ...
  it("rejects rendering when the source follow-up delivery reconciliation chain is tampered", () => {
    const plan = buildManualTriagePlan();
    const receipt = buildManualTriageReceipt();
    const reconciliation = {
      ...buildUnitReviewProviderExecutionFollowUpReconciliation(plan, receipt),
      final_follow_up_queue: "approval_queue",
    } as unknown as UnitReviewProviderExecutionFollowUpReconciliation;

    const validation = validateUnitReviewProviderExecutionFollowUpDeliveryFollowUpSource(
      baseRequest,
      approvedDecision,
      authorizedAttempt,
      failedSourceReceipt,
  ...
  it("detects tampered follow-up delivery follow-up payloads during validation", () => {
    const plan = buildManualTriagePlan();
    const receipt = buildManualTriageReceipt();
    const reconciliation = buildUnitReviewProviderExecutionFollowUpReconciliation(plan, receipt);
    const followUpDeliveryFollowUp = {
      ...buildUnitReviewProviderExecutionFollowUpDeliveryFollowUp(plan, reconciliation),
      follow_up_action: "open_manual_receipt_triage_item" as const,
    };

    const validation = validateUnitReviewProviderExecutionFollowUpDeliveryFollowUp(
      plan,
      reconciliation,
    ...
    expect(validation.issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: "follow_up_action_mismatch" }),
      ])
    );
  });
});
