...
describe("provider execution follow-up", () => {
  ...
  it("hands successful provider execution over to the result artifact contract", () => {
  ...
  it("opens manual execution triage when a provider rerun fails", () => {
    const receipt: UnitReviewProviderExecutionReceipt = {
      schema_version: "content-pipeline-review-provider-execution-receipt/v0.1",
      source_request_schema_version: "content-pipeline-review-provider-execution-request/v0.1",
      source_decision_schema_version: "content-pipeline-review-provider-execution-decision/v0.1",
    ...
    expect(followUp.follow_up_item?.summary).toContain("provider_timeout");
  });

  it("sanitizes failure_code before encoding it into manual-triage labels", () => {
    const receipt: UnitReviewProviderExecutionReceipt = {
      schema_version: "content-pipeline-review-provider-execution-receipt/v0.1",
      source_request_schema_version: "content-pipeline-review-provider-execution-request/v0.1",
      source_decision_schema_version: "content-pipeline-review-provider-execution-decision/v0.1",
      ...
      result_artifact_generated_at: null,
      result_artifact_status: null,
      failure_code: "timeout:provider|edge\ncase"
    };
    ...
    expect(followUp.follow_up_item?.labels).toContain("failure_code:timeout_provider_edge_case");
  });

  it("opens manual receipt triage when reconciliation reports an invalid receipt", () => {
    ...
    const reconciliation: UnitReviewProviderExecutionReconciliation = {
      ...
      receipt_validation_issue_codes: ["artifact_result_contract_mismatch"],
      reconciliation_status: "action_required",
    ...
    expect(followUp).toMatchObject({
      ...
      follow_up_item: {
        human_queue: "manual_triage_queue",
        title: "[Provider Receipt Triage] math_g8_linear_function_intro",
      },
    });
    expect(followUp.follow_up_item?.summary).toContain("artifact_result_contract_mismatch");
  });

  it("rejects malformed reconciliation input before rendering follow-up", () => {
    const invalidReceipt: UnitReviewProviderExecutionReceipt = {
      schema_version: "content-pipeline-review-provider-execution-receipt/v0.1",
      source_request_schema_version: "content-pipeline-review-provider-execution-request/v0.1",
      source_decision_schema_version: "content-pipeline-review-provider-execution-decision/v0.1",
    ...
    const invalidReconciliation: UnitReviewProviderExecutionReconciliation = {
      schema_version: "content-pipeline-review-provider-execution-reconciliation/v0.1",
      source_request_schema_version: "content-pipeline-review-provider-execution-request/v0.1",
      source_decision_schema_version: "content-pipeline-review-provider-execution-decision/v0.1",
      source_attempt_schema_version: "content-pipeline-review-provider-execution-attempt/v0.1",
    ...
    const validation = validateUnitReviewProviderExecutionFollowUpSource(
      baseRequest,
      approvedDecision,
      authorizedAttempt,
      invalidReceipt,
    ...
    expect(validation.issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: "source_reconciliation_contract_mismatch" }),
      ])
    );
  });

  it("reports invalid reconciliation shape instead of throwing on malformed upstream data", () => {
    const receipt: UnitReviewProviderExecutionReceipt = {
      schema_version: "content-pipeline-review-provider-execution-receipt/v0.1",
      source_request_schema_version: "content-pipeline-review-provider-execution-request/v0.1",
      source_decision_schema_version: "content-pipeline-review-provider-execution-decision/v0.1",
    ...
    const malformedReconciliation = {
      ...
      unit_id: baseRequest.unit_id,
      receipt_validation_ok: true,
      receipt_validation_issue_codes: null,
      reconciliation_status: "closed",
      recommended_follow_up: "review_result_artifact",
      execution_outcome: "artifact_recorded",
      result_artifact_available: true,
      ...
      executed_at: "2026-04-24T00:20:00.000Z",
      failure_code: null,
    } as unknown as UnitReviewProviderExecutionReconciliation;
    ...
    expect(validation.ok).toBe(false);
    expect(validation.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ code: "invalid_reconciliation_shape" })])
    );
  });

  it("refuses to build follow-up output from an invalid source reconciliation", () => {
    const receipt: UnitReviewProviderExecutionReceipt = {
      schema_version: "content-pipeline-review-provider-execution-receipt/v0.1",
      source_request_schema_version: "content-pipeline-review-provider-execution-request/v0.1",
      source_decision_schema_version: "content-pipeline-review-provider-execution-decision/v0.1",
    ...
    ).toThrow(/Provider execution follow-up source is invalid/);
  });

  it("detects tampered follow-up payloads during validation", () => {
    const receipt: UnitReviewProviderExecutionReceipt = {
      schema_version: "content-pipeline-review-provider-execution-receipt/v0.1",
      source_request_schema_version: "content-pipeline-review-provider-execution-request/v0.1",
      source_decision_schema_version: "content-pipeline-review-provider-execution-decision/v0.1",
    ...
    const followUp = {
      ...
      automation_step: "open_inbox_item" as const,
    } satisfies UnitReviewProviderExecutionFollowUp;
    ...
    expect(validation.issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: "automation_step_mismatch" }),
      ])
    );
  });
});
